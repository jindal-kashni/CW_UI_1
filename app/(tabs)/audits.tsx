...
import { assetAuditHistory, assetById } from '@/src/data';
...
export default function AuditsScreen() {
  ...
  const [tableView, setTableView] = React.useState<'todo' | 'inprogress' | 'completed'>('inprogress');
  const assignmentRows = assignments.filter((a) => {
    if (tableView === 'todo') return a.status === 'Assigned';
    if (tableView === 'completed') return false;
    return a.status === 'InProgress' || a.status === 'DraftSaved';
  });
  const completedReportRows = tableView === 'completed'
    ? [...assetAuditHistory]
        .sort((a, b) => new Date(b.audit_date).getTime() - new Date(a.audit_date).getTime())
        .map((h) => {
          const linkedAsset = assetById[h.asset_id];
          return {
            id: h.id,
            assetName: linkedAsset?.name ?? 'Condition report submission',
            assetCode: linkedAsset?.asset_code ?? h.asset_id,
            dateLabel: new Date(h.audit_date).toLocaleDateString(),
            progress: 100,
  ...
  const tableRows = tableView === 'completed' ? completedReportRows : assignmentTableRows;
  const rowActionLabel =
    tableView === 'todo' ? 'Begin' : tableView === 'inprogress' ? 'Resume' : 'View';
        ...
        {tableRows.length === 0 ? (
          <Text style={t.text.caption}>
            {tableView === 'todo'
              ...
              : tableView === 'completed'
                ? 'No completed condition reports yet.'
          ...
          <View
            ...
            <View
              ...
              <Text style={[t.text.caption, { flex: 1.1, fontWeight: '700' }]}>
                {tableView === 'completed' ? 'Completed' : 'Due date'}
              </Text>
