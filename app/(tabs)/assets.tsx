...
export default function AssetsScreen() {
  ...
  const activeFilterCount = [category, condition, criticality, status, department, location, room].filter(
    Boolean
  ).length;
  ...
  const filtered = seedAssets
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'conditionLowHigh')
        return ['Excellent', 'Good', 'Fair', 'Poor', 'Dilapidated', 'Critical'].indexOf(a.condition) -
          ['Excellent', 'Good', 'Fair', 'Poor', 'Dilapidated', 'Critical'].indexOf(b.condition);
      return ['Critical', 'High', 'Medium', 'Low'].indexOf(a.criticality) -
        ['Critical', 'High', 'Medium', 'Low'].indexOf(b.criticality);
    });

  const toneForCell = (kind: 'condition' | 'criticality' | 'status', value: string) => {
    if (kind === 'condition') {
    ...
    if (value === 'Under maintenance') return { bg: 'rgba(182,141,61,0.16)', text: '#6A5421' };
    return { bg: 'rgba(179,79,71,0.14)', text: '#7A2E29' };
  };
    ...
    <ScreenScaffold title="Assets">
        ...
        {filterMode === 'search' ? (
          ...
          <View style={{ gap: t.spacing.md }}>
            <View style={{ flexDirection: 'row', gap: t.spacing.md }}>
              {renderDropdownField(filterFields[0])}
              {renderDropdownField(filterFields[1])}
            </View>
            <View style={{ flexDirection: 'row', gap: t.spacing.md }}>
              {renderDropdownField(filterFields[5])}
              {renderDropdownField(filterFields[3])}
            </View>
            <View style={{ flexDirection: 'row', gap: t.spacing.md }}>
              {renderDropdownField(filterFields[2])}
              {renderDropdownField(filterFields[4])}
            </View>
            {location ? (
              <View>{renderDropdownField(filterFields[6])}</View>
            ) : (
              <Text style={t.text.caption}>Select a location to enable room filtering.</Text>
            )}
          </View>
        )}
      ...
      <View
          ...
          {filtered.map((a, idx) => {
            ...
            const statusColors = toneForCell('status', status);
              ...
              <Pressable
                ...
                <View style={{ flex: 1, alignItems: 'center' }}>
                  <View
                    ...
                    <Text style={{ color: statusColors.text, fontSize: 12, fontWeight: '700' }}>{status}</Text>
                  </View>
                </View>
              </Pressable>
            );
